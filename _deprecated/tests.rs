...
fn matrix_operations() {
    ...
    // determinants
    let a: Matrix<f32> = Matrix::new_with_values(2, 2, &[1.0, 5.0, -3.0, 2.0]);
    assert_eq!(a.det(), 17.0_f32);

    // spotting submatrices
    let a = Matrix::new_with_values(3, 3, &[1, 5, 0, -3, 2, 7, 0, 6, -3]);
    let res = Matrix::new_with_values(2, 2, &[-3, 2, 0, 6]);
    assert_eq!(a.submatrix(0, 2), res);

    let a = Matrix::new_with_values(4, 4, &[-6, 1, 1, 6, -8, 5, 8, 6, -1, 0, 8, 2, -7, 1, -1, 1]);
    let res = Matrix::new_with_values(3, 3, &[-6, 1, 6, -8, 8, 6, -7, -1, 1]);
    assert_eq!(a.submatrix(2, 1), res);

    // calculating minors
    let a = Matrix::new_with_values(3, 3, &[3, 5, 0, 2, -1, -7, 6, -1, 5]);
